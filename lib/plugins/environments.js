...
module.exports = class Environments extends Diffable {
  ...
  async sync () {
    ...
    const environments = this.filterEntries()

    console.log('Filtered environments')
    console.log(environments)

    const mappedEnvironments = await this.find()
    console.log('Mapped environments')
    console.log(mappedEnvironments)
  ...
  async getEnvironment (environment) {
    ...
    mapped.name = environment.name.toLowerCase()
    mapped.wait_timer = 0
    mapped.reviewers = []

    // Get the protection rules: wait_timer and required_reviewers
    for (const pr of environment.protection_rules || []) {
      switch (pr.type) {
        case 'wait_timer':
          mapped.wait_timer = pr.wait_timer
          break
        case 'required_reviewers':
          for (const reviewer of pr.reviewers) {
            mapped.reviewers.push({ id: reviewer.reviewer.id, type: reviewer.type })
          }
          mapped.prevent_self_review = pr.prevent_self_review || false
          break
      }
    }
    ...
    if (environment.deployment_branch_policy) {
      mapped.deployment_branch_policy = {
        protected_branches: environment.deployment_branch_policy.protected_branches || false,
        custom_branch_policies: environment.deployment_branch_policy.custom_branch_policies || false
      }
    } else {
      mapped.deployment_branch_policy = null
    }

    // Get custom branch policies
    if (mapped.deployment_branch_policy && mapped.deployment_branch_policy.custom_branch_policies) {
      const policies = await this.getDeploymentBranchPolicies(environment.name)
      for (const policy of policies) {
        mapped.deployment_branch_policy.custom_branch_policies.push({
          name: policy.name,
          type: policy.type
        })
      }
    ...
    const rules = await this.getDeploymentProtectionRules(environment.name)
    for (const rule of rules) {
      mapped.deployment_protection_rules.push({
        app_id: rule.app.id
      })
    }

    // Get environment variables
    const variables = await this.getEnvironmentVariables(environment.name)
    for (const variable of variables) {
      mapped.variables.push({
        name: variable.name,
        value: variable.value
      })
    }
  ...
  async getEnvironments () {
    ...
    const environments = await this.github.repos.getAllEnvironments({
      owner,
      repo
    })

    const mappedEnvironments = Promise.all(environments.map(environment => this.getMappedEnvironment(environment)))

    return mappedEnvironments
  }

  async getDeploymentBranchPolicies (environmentName) {
    const { owner, repo } = this.repo

    const branchPolicies = await this.github.repos.listDeploymentBranchPolicies({
      owner,
      repo,
      environment_name: environmentName
    })

    return branchPolicies.map(({ name }) => ({
      name
    }))
  }

  async getEnvironmentVariables (environmentName) {
    const { owner, repo } = this.repo

    const environmentVariables = await this.github.actions.listEnvironmentVariables({
      owner,
      repo,
      environment_name: environmentName
    })
  ...
  async getDeploymentProtectionRules (environmentName) {
    const { owner, repo } = this.repo

    const deploymentProtectionRules = await this.github.repos.getAllDeploymentProtectionRules({
      owner,
      repo,
      environment_name: environmentName
    })

    return deploymentProtectionRules.map(({ id, app }) => ({
      id,
      app_id: app.id
    }))
  }
